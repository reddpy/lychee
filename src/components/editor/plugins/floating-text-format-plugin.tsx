...
function FloatingTextFormat({
  ...
  const toolbarButtonClass =
    "inline-flex h-8 w-8 min-w-8 items-center justify-center rounded-md text-gray-700 transition-colors hover:bg-gray-100 data-[state=on]:bg-gray-200 data-[state=on]:text-gray-900"

  return (
    <div
      ref={popupCharStylesEditorRef}
      className="absolute top-0 left-0 z-50 flex gap-1 rounded-md border border-gray-200 bg-white p-1 text-gray-900 opacity-0 shadow-lg transition-opacity duration-300 will-change-transform [color-scheme:light]"
    >
      {editor.isEditable() && (
        <>
          <button
  ...
  useEffect(() => {
    return mergeRegister(
      ...
      editor.registerCommand(
        SELECTION_CHANGE_COMMAND,
        () => {
          // Defer so we read selection after Lexical has committed
          queueMicrotask(updatePopup)
          return false
        },
        COMMAND_PRIORITY_LOW
      ),
